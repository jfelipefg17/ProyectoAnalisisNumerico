import numpy as np


def forward_substitution(M):
    """
    Solves a lower triangular system using forward substitution.
    The input matrix M must be the augmented matrix [L | b].
    """
    n = M.shape[0]
    x = np.zeros(n)

    x[0] = M[0, n] / M[0, 0]

    for i in range(1, n):
        s = np.dot(M[i, :i], x[:i])
        x[i] = (M[i, n] - s) / M[i, i]

    return x


def backward_substitution(M):
    """
    Solves an upper triangular system using backward substitution.
    The input matrix M must be the augmented matrix [U | b].
    """
    n = M.shape[0]
    x = np.zeros(n)

    x[n - 1] = M[n - 1, n] / M[n - 1, n - 1]

    for i in range(n - 2, -1, -1):
        s = np.dot(M[i, i + 1:n], x[i + 1:n])
        x[i] = (M[i, n] - s) / M[i, i]

    return x


def cholesky(A, b):
    """
    Solves Ax = b using Cholesky factorization (A = L * L^T).

    Parameters
    ----------
    A : array-like — symmetric positive definite matrix (n x n)
    b : array-like — right-hand side vector (n)

    Returns
    -------
    x      : numpy.ndarray — solution vector
    L      : numpy.ndarray — lower triangular matrix
    U      : numpy.ndarray — upper triangular matrix (U = L^T)
    stages : list of dicts — intermediate L and U at each step
    """

    # --- Convert inputs to numpy arrays ---
    A = np.array(A, dtype=float)
    b = np.array(b, dtype=float)

    # --- Validations ---
    if A.shape[0] != A.shape[1]:
        raise ValueError("Matrix A must be square")

    n = A.shape[0]

    if b.shape[0] != n:
        raise ValueError("Vector b size must match A")

    if not np.allclose(A, A.T):
        raise ValueError("Matrix A must be symmetric")

    if np.linalg.det(A) == 0:
        raise ValueError("Matrix A must be non-singular")

    eigenvalues = np.linalg.eigvalsh(A)
    if np.any(eigenvalues <= 0):
        raise ValueError("Matrix A must be positive definite (all eigenvalues > 0)")

    # --- Initialization ---
    L = np.eye(n)
    U = np.eye(n)

    stages = [{"A": A.copy()}]

    # --- Cholesky Factorization ---
    for i in range(n - 1):

        # Diagonal element: L[i,i] = sqrt(A[i,i] - sum(L[i,k]^2, k<i))
        inner = A[i, i] - np.dot(L[i, :i], U[:i, i])
        if inner <= 0:
            raise ValueError(
                f"Non-positive value under square root at step {i}. "
                "Matrix may not be positive definite."
            )
        L[i, i] = np.sqrt(inner)
        U[i, i] = L[i, i]

        # Column i of L and row i of U
        for j in range(i + 1, n):
            L[j, i] = (A[j, i] - np.dot(L[j, :i], U[:i, i])) / U[i, i]

        for j in range(i + 1, n):
            U[i, j] = (A[i, j] - np.dot(L[i, :i], U[:i, j])) / L[i, i]

        stages.append({"L": L.copy(), "U": U.copy()})

    # Last diagonal element
    inner_last = A[n - 1, n - 1] - np.dot(L[n - 1, :n - 1], U[:n - 1, n - 1])
    if inner_last <= 0:
        raise ValueError(
            "Non-positive value under square root at last step. "
            "Matrix may not be positive definite."
        )
    L[n - 1, n - 1] = np.sqrt(inner_last)
    U[n - 1, n - 1] = L[n - 1, n - 1]

    # --- Forward substitution: Lz = b ---
    z = forward_substitution(np.column_stack((L, b)))

    # --- Backward substitution: Ux = z ---
    x = backward_substitution(np.column_stack((U, z)))

    return x, L, U, stages
